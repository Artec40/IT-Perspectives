...
import {ArticleTypeSelector, EmployeeTypeSelector} from "../../../types/types";

type PropsType = {
    employees: Array<EmployeeTypeSelector>
    articles: Array<ArticleTypeSelector>
}
const Team: React.FC<PropsType> = ({employees, articles}) => {

    let renderEmployees = (employees: Array<EmployeeTypeSelector>) => {
        return employees.map((e) => <Employee key={e.id} id={e.id}
                                              image={e.image} name={e.name}/>)
    }
    let renderArticles = (articles: Array<ArticleTypeSelector>) => {
